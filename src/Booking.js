    ...
    return (
        <div className="flex h-screen p-4">
            ...
            <div className="flex-1 flex-col">
                ...
                <div className='px-4 my-4 flex justify-between items-center'>
                    <div className="relative w-full max-w-sm">
                        <input
                            type="text"
                            placeholder="Search"
                            className="w-full bg-[#D9D9D9] placeholder-black placeholder:font-semibold pl-4 pr-10 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-black"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                        />
                        <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 text-black" size={20} strokeWidth={2.5} />
                    </div>
                ...
                <h1 className='font-semibold text-black text-xl px-4 my-4'>Good Morning Mr. Naval Ravikant !</h1>
                ...
                <div className='grid grid-cols-3 gap-4 px-4 '>
                    ...
                    {filteredMovies.map((movie) => (
                        <div
                            ...
                            className="cursor-pointer flex flex-col justify-center items-start bg-white rounded-lg"
                        >
                            <img
                                className="h-48 object-fill rounded-md w-full border border-black"
                                src={movie.image}
                                alt={movie.title}
                            />
                            <h1 className="text-black text-lg font-semibold px-2 my-1">
                                {movie.title}
                            </h1>
                        </div>
                    ))}
